...
for example, they cannot match brackets in the general case.
...
What distinguishes one langauge from another is restrictions placed on grammars.

A grammar defines a language: it generates the set of strings in the language
and tests membership of a string in the language.

A *grammar* is a set of nonterminals (variables), terminals (words or symbols),
