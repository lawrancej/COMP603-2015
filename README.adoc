...
*Dynamically-typed* languages do this at run-time (e.g., Python, Ruby, Javascript).
The interpreters for these languages check for type mismatches at runtime.
*Statically-typed* languages do this at compile-time (e.g., Java, C++, Haskell).
The compilers for these languages traverse the AST to verify no types are mismatched.
